...
console.log(`Extraction eval: ${files.length} screenshot(s), model ${verifier.modelId}, today ${defaultToday}\n`);
...
for (const file of files) {
  ...
  try {
    const result = await verifier.extract({
...
if (process.env.GITHUB_STEP_SUMMARY) appendFileSync(process.env.GITHUB_STEP_SUMMARY, renderMarkdown(rows, heading));
